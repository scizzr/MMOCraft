...
public class Barbarian implements Race {
    ...
    public void attackRight(Player p, Action a) {
        ...
        int dmg = Util.calcBaseDamage(p.getItemInHand()) + 6;
        ...
        if (Util.classWep(RaceMgr.getRace(p.getName()), p.getItemInHand().getTypeId())) {
            ...
            if (p.getLocation().getPitch() <= -60) {
                new BarbToughSkin().execute(p, null, (int)dmgClass);
            } else if (p.getLocation().getPitch() >= 60) {
                new BarbLeap().execute(p, null, (int)dmgClass);
            } else {
                if (a == Action.RIGHT_CLICK_AIR) {
                    new BarbAxeWhirl().execute(p, null, (int)dmgClass);
                }
            }
        }
    }
    ...
    public void attackEntity(Player p, EntityDamageByEntityEvent e) {
        ...
        if (Util.classWep(race, p.getItemInHand().getTypeId()) && p.getGameMode() == GameMode.SURVIVAL) {
